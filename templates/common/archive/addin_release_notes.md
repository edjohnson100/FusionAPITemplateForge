...
```
git archive --format=zip --prefix={{NAME}}/ -o {{NAME}}-v1.0.0.zip HEAD
```

- Swap `HEAD` for a tag (e.g. `v1.0.1`) once tags are actually being created, so the zip always matches the tagged release rather than whatever's currently checked out.
- Bump the output filename's version to match each release.
- Note: this only works once the v1.1.0 tag actually exists (git tag v1.1.0 on the commit you want, then git push origin v1.1.0 if you want it on GitHub too) — running it against a tag that doesn't exist yet fails with something like fatal: v1.1.0 is not a valid tree.

```
git archive --format=zip --prefix={{NAME}}/ -o {{NAME}}-v1.1.0.zip v1.1.0
```
...
## v1.0.0 -- Unreleased
- Initial scaffold from the Fusion_Addins template.
